...
_STEP_HUMAN = {
    "approve_ttk": (
        ...
        "就像你在网购时授权支付宝从你银行卡扣款一样。这一步完成后，托管合约才有权动用你的代币作为任务报酬。"
    ),
    "create_job": (
        "在链上创建托管任务",
        "在 ERC-8183 合约上正式登记一个任务：指定谁来做（服务商）、谁来验收（裁决者）、任务什么时候到期。"
        "好比你在平台上发布了一个悬赏任务，写明要求和截止时间。"
    ),
    "set_budget": (
        "设置任务赏金预算",
        "为刚创建的任务设定报酬金额。比如「这个任务完成后，服务商能拿到 100 TTK」。"
        "就像你发布任务时设定悬赏金额。"
    ),
    "fund": (
        "将赏金锁定到托管合约",
        ...
        "就像把悬赏金交给平台暂存，交易完成才打款。"
    ),
    "submit": (
        "服务商提交工作成果",
        "服务商（Provider）完成工作后，在链上提交一个工作证明哈希。这相当于说「我做完了，请验收」。"
        "此时资金仍然锁定在托管合约中，要等裁决者验收通过才会放款。"
    ),
    "complete": (
        "裁决者验收通过，放款给服务商",
        "裁决者（Evaluator，由 LLM 自动评判）确认工作合格，批准将托管合约中的赏金转给服务商。"
        "就像你在平台上点「确认收货」，钱从平台打给卖家。"
    ),
    "reject": (
        "裁决者验收不通过，退款给客户",
